 ...
 * annotation. So this factory can contains some {@link cz.zcu.kiv.jop.factory.binding.Binding
 * Bindings} between annotations and classes. Method {@link #createInstance(Annotation)} provides
...
public interface BindingFactory<T> extends Factory<T> {
   ...
   * annotated by {@link javax.inject.Singleton Singleton} annotation it will be cached so the
   * returned instances will be singletons. Otherwise the factory always creates new instance for
   * each call of this method.
   * <p>
   * In case that there is no bound implementation of interface <code>T</code> for given annotation
   * the exception may be thrown - the <code>null</code> value should not be returned.
   *
   * @param annotation the annotation for which will be created bound instance of interface
   *          <code>T</code> implementation.
   * @return Created instance of interface <code>T</code> implementation which was bound to given
   *         annotation.
