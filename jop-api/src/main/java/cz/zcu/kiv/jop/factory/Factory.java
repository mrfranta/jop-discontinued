...
/**
 * Common interface for factory which serves for creation of instances of
 * implementations for generic (interface) type <code>T</code>. Implementations
 * of this interface should be created primary by this factory. If a
 * implementation is annotated by {@link javax.inject.Singleton Singleton}
 * annotation the created instances will be cached so returned instances will be
 * singletons. Otherwise the factory always creates new instance for each
 * creation of instance for implementation.
 *
...
public interface Factory<T> {
   ...
   *
   * @param clazz the class type for implementation of interface <code>T</code>
   *          which instance will be (created and) returned.
