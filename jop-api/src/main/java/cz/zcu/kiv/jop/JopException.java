...
/**
 * Common exception for all exceptions in this library which provides the common behavior for them
 * and also brings possibility to catching exceptions in one simple catch block. Also it brings
 * possibility to more flow of exception through multiple method calls.
 *
 * @author Mr.FrAnTA
 * @since 1.0.0
 */
public class JopException extends Exception {
  ...
  /**
   * Constructs a new exception with the specified cause and <code>null</code> as its detail
   * message. This constructor is different to parent {@link Exception#Exception(Throwable)
  ...
  /**
   * Returns the detail message string of this exception. If message is <code>null</code> (because
   * of complex constructors) and there is no default message ({@link #getDefaultMessage}), the
   * message from cause will be returned.
   *
   * @return The detail message string of this {@link JopException} instance.
   */
  @Override
   ...
   * <code>null</code>. The default implementation returns also <code>null</code> and may be
   * overridden in child classes.
   *
   * @return Always <code>null</code>. In case of override, it returns default message of exception.
   */
